...
class BuildPosIndexDialog(QDialog):
    ...
    def __init__(self, client: BackendClient, default_ply: int = 24, parent=None):
        ...
        self.btn_build.setStyleSheet(
            "font-weight: bold; background-color: #2e7d32; color: white; padding: 6px 16px;"
        )
    ...
    def start_build(self):
        ...
        if not self.queue:
            QMessageBox.warning(
                self, "No Index Selected", "Please select at least one index to build."
            )
            return
    ...
    def open_diagnostics(self):
        ...
        diag = PosIdxDiagnosticsDialog(
            self.client, initial_data=self.last_diagnostics, parent=self
        )
        diag.exec_()
